...
UPDATE Sales SET sales_code = CONCAT('SAL-', DATE_FORMAT(sales_date, '%y%m%d'), '-', LPAD(sales_id, 5, '0')) WHERE sales_id = LAST_INSERT_ID();
...
UPDATE Expense SET expense_code = CONCAT('EXP-', DATE_FORMAT(expense_date, '%y%m%d'), '-', LPAD(expense_id, 5, '0')) WHERE expense_id = LAST_INSERT_ID();
-- 2
INSERT INTO Expense (warehouse_name, expense_date, category, amount, description)
...
UPDATE inbound
SET
    inbound_status = 'approved', -- 사용자 요청 상태 'approved'
...
VALUES ('2025-10-18 10:00:00', 1, 1, 1, 'Approved', '서울시 강남구'), -- 사용자 요청 상태 'Approved'
       ('2025-10-20 13:00:00', 2, 2, 2, 'Approved', '서울시 서초구'); -- 사용자 요청 상태 'Approved'
...
VALUES ('2025-11-03 11:00:00', 3, 3, 3, 'Approved', '서울시 마포구'), -- 사용자 요청 상태 'Approved'
       ...
       ('2025-11-09 16:00:00', 6, 6, 6, 'Approved', '서울시 종로구'); -- 사용자 요청 상태 'Approved'